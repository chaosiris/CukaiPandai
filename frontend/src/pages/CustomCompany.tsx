...
// Captures all EntityTaxProfile fields, calls addCustomPersona (JR-1 → localStorage),
// then best-effort POSTs to /entities (BE-J1). FE does NOT block on the server write.
...
import { type SsmProfile, createEntity } from '../api/client'
...
export default function CustomCompany() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Step 1: persist locally + set active (must not fail)
    addCustomPersona(persona)

    // Step 2: best-effort BE write — FE does not block on this
    setServerError(null)
    createEntity(ssm).catch((err: Error) => {
      // Surface a friendly note but keep local entity fully functional
      setServerError(`Note: could not save to server (${err.message}). Your company is still active locally.`)
    })

    // Navigate immediately — local entity is already active
    ...
    <div style={{ maxWidth: 640, margin: '0 auto', padding: '32px 0 80px' }}>
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontFamily: 'var(--font-body)', fontSize: 14, color: 'var(--ink-soft)', lineHeight: 1.5 }}>
          ...
          figures. Data is stored locally in your browser; no account required.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} noValidate>
        ...
        >
          YA2026 · decision-support, not legal advice · data stored locally in your browser
        </p>
      </form>
    </div>
  )
