...
**Fixed account taxonomy + full ascertainment chain (SFI, 27/06/26).** Line items key off a canonical chart of **88 fixed tax accounts across 14 groups** (`core/tax_accounts.py`, mirrored on the FE in `lib/taxAccounts.ts`); each account carries a `category` ∈ {`income`, `exempt_income`, `deductible`, `non_deductible`, `capital_allowance`, `special_deduction`} plus `ca_class`/`relief_key` engine keys. `compute_form_c` runs the YA2026 chain: business income − allowable deductions − further/double deductions = **adjusted income** → + balancing charge − Schedule-3 capital allowances (IA+AA per `ca_class`, MV cost cap, SVA SME-uncapped, IBA; renovation = no auto-allowance) = **statutory income** → − current-year loss = **aggregate income** → − b/f loss − zakat (≤2.5% of aggregate income) − approved donations (≤10% of aggregate income) − group relief = **total = chargeable income** (a company has no personal reliefs) → SME two-tier / flat 24% = **tax payable**. Exempt credit items (single-tier dividend, gain on disposal, unrealised forex gain) are excluded from taxable income. All eight stages are emitted as `FormComputation.fields` traces; CA rates + relief caps live cited in `config/ya_2026.yaml` (`capital_allowances` + `reliefs`). The golden `chargeable_income RM200,000 → tax_payable RM31,000` reduces unchanged.
...
`POST /entities` (onboard → profile) · `GET /entities/{tin}` (entity profile — returns seeded `EntityTaxProfile`; 404 on unknown TIN; **BE-8**) · `POST /entities/{tin}/obligations` (calendar) · `POST /entities/{tin}/filings/form-c` (compute → cited `FormComputation` + `risk_flags` + `sovereign`/`active_model`) · `POST /entities/{tin}/filings/form-c/start` + `/resume` (HITL: pause at the human-approval interrupt → resume with the approval decision; **shipped, BE-2**) · `POST /entities/{tin}/audit-defense` (query → `DefensePack`) · `POST /entities/{tin}/documents/classify` + `POST /entities/{tin}/documents/upload` (raw text / uploaded Income Statement·P&L·Trial Balance → `LineItem[]`, JSON-mode, 502-guarded; **constrained to the 88-account taxonomy — the agent maps each row to a known code and drops non-financial noise; the server drops codes outside the taxonomy + non-finite/zero amounts and sets `category` authoritatively from the taxonomy, never the model**; **BE-9 / SFI-5**). Structured manual entry on `/filing/new` is **deterministic (no LLM)** — the user picks accounts from the taxonomy and `LineItem`s are built directly. · `GET /me/filings/{id}/report` (owner-scoped → inline `application/pdf` Form C **draft pack** rendered by WeasyPrint from the saved computation; a preparation aid, **never submitted** — self-assessment via MyTax; 404/409/503; **SFI-9**) · `GET /reference/msic/{code}` (MSIC reference lookup; **shipped, BE-4**) · `GET /entities/{tin}/evidence` · CORS middleware via `FRONTEND_ORIGIN`/`CORS_ORIGINS` env (**BE-7**); all validation errors return a consistent **422** with field detail (**BE-10**); AI-backed responses carry a `sovereign: bool` + `active_model: str` field (**BE-6**). SSE `/runs/{id}/stream` for live agent steps (STRETCH, BE-11). _(Shipped routes: `GET /health`, `POST …/obligations`, `POST …/filings/form-c`, `POST …/filings/form-c/start`, `POST …/filings/form-c/resume`, `POST …/audit-defense`, `GET /reference/msic/{code}`.)_
