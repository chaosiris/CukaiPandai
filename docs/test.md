...
## [27/06/26] — Branding pass (PR-D): Talina wordmark · auth logo · FAQ mascot · mobile HOW IT WORKS · social meta `[FE]`

**Branch:** `feat/branding` (uncommitted vs `main`). Ad-hoc branding pass, no plan task id. Changed (tracked): `frontend/index.html`, `frontend/src/pages/{AuthScreen.tsx,Auth.css,Landing.tsx,Landing.css}`, `frontend/src/styles/tokens.css`, `docs/progress.md`. **Untracked deletion+move:** `docs/runbook.md` (deleted, 189 lines) → `runbook.md` at **repo root** (new). New assets: `frontend/public/fonts/talina.otf`, `frontend/public/pandai-thinking.png`, `frontend/public/og-banner.png`.

**Verdict:** Reject with reasons

All three gates pass and four of the five branding requirements are implemented correctly. But the diff carries **two out-of-scope problems** that must not land in a branding PR: (M1) `docs/runbook.md` has been **deleted and moved to the repo root** — a destructive, unrelated change to operational docs; and (M2) the mobile HOW-IT-WORKS rewrite introduces a **shared-ref collision that breaks the desktop active-step emphasis** (a real desktop regression, which the brief explicitly told me to reason about). Both are fixable quickly. The font/auth/FAQ/social work itself is sound.

### Smoke test / gates (all PASS)

- `cd frontend && bunx tsc --noEmit` → **exit 0, clean.**
- `cd frontend && bun run build` (`tsc -b && vite build`) → **83 modules transformed, dist/ emitted, exit 0.** All four branding assets copy into `dist/` (`og-banner.png`, `pandai-thinking.png`, `fonts/talina.otf`, `logo.png` present).
- `bunx biome check frontend/src` → **"Checked 46 files, no fixes applied", exit 0.**

### Requirement-by-requirement

1. **Talina on the wordmark ONLY — PASS.** `@font-face` for "Talina" added (`tokens.css:3-9`, `font-display: swap`, opentype). `--font-brand: "Talina", "Fraunces", Georgia, serif` added (`tokens.css:37`) with the required **Fraunces fallback** in the stack. `var(--font-brand)` is applied to exactly the four wordmark classes and nowhere else: `.topbar-wordmark` (`tokens.css:291`), `.drawer-wordmark` (`:499`), `.footer-wordmark` (`:1172`), `.auth-brand` (`Auth.css:26`). Grep-verified: **no `--font-display` rule and no general heading was switched to Talina** — all ~40 other `var(--font-display)` usages (headings, window titles, FilingPipeline/JourneyProgress/Welcome/etc.) are untouched. `--font-display` itself is unchanged (`tokens.css:33`). The `.brand-lockup .topbar-wordmark/.drawer-wordmark` rule (`:248-249`) sets only `color`, so no font conflict. Correct and surgical.

2. **Auth-page real logo — PASS (with one minor orphan).** `AuthScreen.tsx:127` now renders `<img src="/logo.png" alt="CukaiPandai" className="brand-logo" />`; the `LogoMark` import is removed (`-import { LogoMark } …`). `.auth-brand .brand-logo` sizing rule present at 26px (`Auth.css:31`); the old `.auth-brand .logo-mark` rule is gone (no dead auth-scoped rule). `alt="CukaiPandai"` is correct/non-decorative for a brand logo. `/logo.png` exists in `public/`. **Minor:** the change orphans the global `.logo-mark` rule (`tokens.css:253-260`) and the `LogoMark` component (`icons.tsx:6`) — AuthScreen was their last consumer (grep confirms zero remaining `<LogoMark` / `logo-mark` usages anywhere in `src`). TS does not flag `LogoMark` because it is `export`ed. This is dead code the change created; per the brief's "no dead `.logo-mark` rule left" check it should be cleaned, but it is non-blocking (harmless, and the orphan rule is global not auth-scoped).

3. **Landing FAQ mascot + cleanup — PASS.** Two-column layout added: `.lp-faq-mascot-col` is `aria-hidden="true"` with `<img … alt="">` (decorative, correct double-signal). "See All Questions" `<Link to="/faq">` **removed**; the `lp-script` "every figure has a citation." tagline **removed**. "Open the Demo" CTA (`lp-finale-cta to="/sign-in"`) and the dark finale band (`.lp-finale { background:#1c1b19 }`, `Landing.css:496-498`) are both **preserved**. Mobile: at ≤900px `.lp-faq-inner` collapses to 1-col and the mascot shrinks 220px→120px — stacks acceptably. **Minor:** removing the markup orphans two CSS rules (`.lp-faq-more` `Landing.css:661`, `.lp-script` `:509`) — dead CSS, non-blocking; same orphan-cleanup category as #2. `/faq` route is NOT orphaned (still reachable elsewhere; only the link was dropped).

4. **Mobile HOW IT WORKS stepped presentation — REJECT (M2, desktop regression).** Two sibling layouts now exist (`.lp-how-desktop` grid >900px / `.lp-how-mobile` block ≤900px) and the mobile one correctly interleaves each step header with its own console mock and reuses `activeStep`. **But both layouts attach refs to the SAME indices** `cardRefs.current[0..2]` — desktop on the card slots (`Landing.tsx:191`), mobile on the `<li>`s (`:219`). Both subtrees are always mounted (visibility is CSS `display:none`, not conditional render), and React runs ref callbacks in mount order, so the **mobile callbacks run last and overwrite** indices 0-2 with the `<li>` elements. On desktop the mobile `<li>`s are `display:none` → `getBoundingClientRect().top` returns **0** for all three. The scroll handler (`:122-128`) does `if (top <= trigger) idx = i` for every ref, so `0 <= trigger` is true for all → `idx` ends at **2 on every scroll** → desktop active-step emphasis is **stuck on step 3** (the sticky-nav highlight the desktop layout depends on is broken). This is a genuine desktop regression introduced by the shared-ref design — exactly the duplicate-ref/scroll-conflict the brief asked me to reason about. The `cardRefs` type widening `HTMLDivElement → Element` is itself sound (it must hold both `<div>` and `<li>`); the bug is the shared index space, not the type. → **Fix:** give each layout its own ref array (e.g. `desktopRefs` / `mobileRefs`) and have `onScroll` measure whichever array's first element is actually rendered (offsetParent !== null), OR gate the inactive layout with a viewport check so only one set of refs is live. (Note: even today the mobile layout _does_ work in isolation at ≤900px because there the desktop slots are the hidden ones — but desktop, the default, is broken.)

5. **Social embed meta — PASS, with the og:url flag the brief requested.** `index.html` adds `og:type=website`, `og:url`, `og:title`, `og:description`, `og:image=/og-banner.png` (+ `og:image:width=1200`/`height=630`), and `twitter:card=summary_large_image`, `twitter:title`, `twitter:description`, `twitter:image=/og-banner.png`. All present and pointing at `/og-banner.png` (961 KB; large but within crawler limits; optional optimization). **NEEDS-CONFIRM (non-blocking):** `og:url` is `https://cukaipandai.my/` — the app was previously referenced at `cukaipandai.vercel.app`. The PO must confirm the canonical production domain before merge; a wrong canonical URL degrades link unfurls but does not break the build. **Minor (note):** `og:image`/`twitter:image` use a **root-relative** path (`/og-banner.png`); some social crawlers (older Facebook/LinkedIn parsers) require an **absolute** URL for the image — recommend `https://<canonical-domain>/og-banner.png` once the domain is confirmed in the same step as the og:url fix.

### Findings (severity-tagged)

- **[MAJOR — M1, out-of-scope destructive] `docs/runbook.md` deleted → moved to repo root `runbook.md`.** `git diff main` shows `docs/runbook.md | 189 -------` (file no longer exists on disk); an **untracked** `runbook.md` at the repo root is **byte-identical** to `main:docs/runbook.md` (verified via `diff -q`). This is wholly unrelated to branding, removes a tracked operational doc from its canonical `docs/` location, and would either (a) land as a deletion of `docs/runbook.md` if committed, or (b) silently leave the move half-done. CI/DO and the runbook's own deploy instructions reference `docs/runbook.md`. → **Fix:** restore `docs/runbook.md` (`git checkout main -- docs/runbook.md`) and delete the stray root `runbook.md`; keep it entirely out of this PR. If a docs move is actually wanted, it belongs in a separate `docs:` PR with all references updated.
- **[MAJOR — M2, desktop regression] `Landing.tsx:191 + :219` shared `cardRefs` index space.** Desktop active-step emphasis breaks (stuck on step 3) because the always-mounted hidden mobile `<li>`s overwrite the same ref indices and report `rect.top === 0`. Detail + fix above (req #4).
- **[MINOR] `tokens.css:253-260` `.logo-mark` rule + `icons.tsx:6` `LogoMark` are now orphaned** by the AuthScreen change (last consumer removed). Dead code the change created. → remove both, or note for a cleanup pass.
- **[MINOR] `Landing.css:661 .lp-faq-more` + `:509 .lp-script` are now orphaned CSS** (markup removed in req #3). → remove.
- **[MINOR/NEEDS-CONFIRM] `index.html` `og:url=https://cukaipandai.my/`** — confirm the canonical domain (prior refs used `cukaipandai.vercel.app`). Also make `og:image`/`twitter:image` absolute once the domain is fixed.
- **[INFO] `og-banner.png` is 961 KB** — within OG limits but heavier than needed; optional optimization (the mascot was optimized 1.5 MB→142 KB per progress.md; the banner was copied as-is).

### Verified clean (no action)

- **Talina scoping is exact** — only the 4 wordmark classes; no heading/`--font-display` switched (grep-confirmed across all of `src`).
- **Auth a11y** — brand logo has a real `alt`; FAQ mascot is `aria-hidden` + `alt=""` (decorative). No a11y regressions.
- **Finale CTA + dark band preserved**; `/faq` route not orphaned.
- **No AI attribution** in the diff (no co-authored / generated-with / noreply@anthropic / emoji markers).
- **Surgical within FE** apart from the runbook move (M1): no tax figures, citations, core math, or backend touched.

**Return to PM:** **REJECT.** Branding substance is good — Talina is correctly scoped to the four wordmark classes only (Fraunces fallback intact, no heading hijacked), the auth page uses the real `/logo.png` with a proper `alt`, the FAQ mascot is decoratively wired (`aria-hidden`+`alt=""`) with "See All Questions" and the script tagline removed while the "Open the Demo" CTA + dark finale band are preserved, and the OG/Twitter meta is complete. **All three gates pass** (tsc 0, build 83 modules, biome 0). **Two blockers, both quick fixes:** (M1) `docs/runbook.md` was deleted and moved verbatim to repo root `runbook.md` — out-of-scope and destructive, restore it and drop the stray; (M2) the mobile HOW-IT-WORKS rewrite shares `cardRefs` indices between the always-mounted desktop and mobile layouts, so the hidden mobile `<li>`s (rect.top=0) overwrite the refs and **break desktop's active-step emphasis** (stuck on step 3) — give each layout its own ref array. Plus the og:url domain (`cukaipandai.my` vs prior `cukaipandai.vercel.app`) needs PO confirmation and the og:image should be absolute. Loop back to PG for M1 + M2 (and ideally the four minor orphan/dead-code + absolute-og cleanups), then re-QA.

---

## QA Verdict — PR-E (post-#27 frontend fixes, `feat/pr27-fixes`) — 27 Jun 2026

**Verdict:** APPROVE

All five fixes are implemented correctly, surgically, and within scope. All three gates pass clean. No blocking issues. Two of the five fixes (divider full-bleed, solid backgrounds) are visual and warrant a quick post-merge browser smoke test, but the code-level reasoning holds in both light and dark themes. Reviewed `git diff main -- frontend/ docs/` (7 files: Analytics, CustomCompany, FilingNew, FilingRecord, FilingStudio, tokens.css, progress.md).

### Smoke test / gates (all PASS)

- `cd frontend && bunx tsc --noEmit` → **exit 0, clean.**
- `cd frontend && bun run build` (`tsc -b && vite build`) → **83 modules transformed, dist/ emitted, built in 2.09s, exit 0.**
- `bunx biome check frontend/src` (from repo root) → **"Checked 46 files, no fixes applied", exit 0.**

### Fix-by-fix

1. **Row-divider full-bleed (Fix 1) — PASS.** `tokens.css` switches `.row-list > * + *`, `.row-div-list > * + *`, and `.requirement-row + .requirement-row` from `border-top: var(--border)` to `box-shadow: inset 0 1px 0 var(--ink)` (`tokens.css:860, 1764, 1775`). The divider color is preserved: old `--border` = `1px solid var(--ink)`, new uses the same `--ink` (full near-black `#1c1b19` light / off-white `#e9edf3` dark) — not a softer tone, so visual weight is unchanged. **Full-bleed reasoning:** the inset box-shadow is painted across the full border-box of the row child, unaffected by the child's own padding — so it reaches both the left and right inner edges. I verified every consumer wraps its rows in an **unpadded** container that is a direct child of `.window` (which itself has no padding — `tokens.css:104-111`, only border+radius), with the rows carrying their own padding: Analytics `.row-list` ×3 (`Analytics.tsx:115,264,309`), FilingRecord `.row-div-list` (`:247`), FilingNew `.row-div-list` ×2 (`:565,650`), FilingStudio (`:334`), AuditAssistant ×2 (`:646,791`), and `.requirement-row` consumers (CitationPanel, FilingPipeline, ComputationPanel req-list). **No consumer wraps rows in a padded container that would re-inset the line.** `:last-child { border-bottom: none }` is now a harmless safety guard (the `!important` was dropped — no competing rule sets a bottom border, so this is fine). Window `border-radius: 3px` only affects the outer top/bottom corners, not interior dividers — no clipping. → _Post-merge: visual confirm the line touches both edges in-browser._
2. **PENDING draft resume (Fix 2) — PASS.** `FilingNew.tsx:164-182` rehydrates a synthetic `ClassifyResponse` (`{ line_items, sovereign: true, active_model: 'draft' }` — matches the `ClassifyResponse = RouteInfo + line_items` type at `client.ts:118-138`), calls `setClassifyResult`/`setLineItems`/`setPhase({tag:'classified'})` (a valid Phase variant, `FilingNew.tsx:68`), and still restores `rawText` + `draftId`. **No-duplicate verified:** because resume restores `draftId = rec.id`, the compute path takes the `upgradeFiling(targetId, …, status:'final')` branch (`FilingNew.tsx:337-345`) and upgrades the SAME record — no new draft created. **Mock round-trip verified:** `createDraftFiling` persists `line_items`+`raw_text` into `_mockFilings` (`client.ts:939-940`) and `getFiling` returns the same record (`:980-983`), so `VITE_API_MOCK=1` demos the classified resume end-to-end. **Edge case handled:** empty/missing `line_items` fails the `rec.line_items && rec.line_items.length > 0` guard → phase stays `idle` (re-classify), only rawText/draftId restored — no crash, no blank classified stage. A missing draft is caught and starts fresh. _Note (pre-existing, not introduced by this PR): a separate persona-reset effect (`:185-192`, deps `[persona.tin]`) resets state to idle; on mount the resume wins because `getFiling().then()` resolves after the synchronous reset, but switching persona while `?resume=` is in the URL would clobber the resumed draft. This interplay predates PR-E and is out of scope._
3. **Provenance into heading InfoTip (Fix 3) — PASS.** The standalone `.window` "[i] How this was calculated" card is removed from `FilingRecord.tsx`; the same text is passed as `headingTip` to `ComputationPanel` (`FilingRecord.tsx:211-218`). `ComputationPanel` renders `{headingTip && <InfoTip content={headingTip} label="How this was calculated" />}` next to the `titlebar-title` span (`FilingPipeline.tsx:230-233`) — an InfoTip on the card title, **never on an `<h1>`**, with an accessible label. The `InfoTip` import in FilingRecord is **not orphaned** (still used at `:226, :242`). `headingTip?: ReactNode` is typed and `ReactNode` is imported in FilingPipeline.
4. **Analytics CTA cleanup (Fix 4) — PASS.** The bottom `borderTop` divider div and both CTA `Link`s ("Open Obligation Calendar", "Start Form C Filing") are removed (`-21` lines). The `Link` import is **retained and still used** by the entity-setup empty state (`Analytics.tsx:387 to="/entity"`). No orphan styles/handlers introduced (the removed block was inline-styled).
5. **Solid-background sweep (Fix 5) — PASS.** Five over-grid bordered controls changed `background: 'transparent'` → `'var(--window)'` while keeping their borders: FilingNew "Edit Trial Balance" (`:604`, `border: var(--border)`), FilingRecord "Back to Filing Records" (`:147`) + "All Filings" (`:265`), FilingStudio "Delete Selected" (`:231`, `1px solid var(--rust)` border kept), CustomCompany "Back" (`:462`). I confirmed the FilingStudio toolbar is a bare `<div>` over the grid (`:188`), so the fill is correct (not redundant). **Contrast holds both themes:** `--window` (`#f7f3e9` light / `#1d2430` dark) with `--ink`/`--ink-soft`/`--rust` borders+text gives a clear lift off the `.page-scroll` grid in both. **Intentional transparency correctly left alone:** the remaining 9 `transparent` backgrounds are inside `.window`/dialogs/popovers (Tooltip, AuditAssistant, AppShell), wizard layouts, or are borderless text-link skip buttons. Spot-checked `Welcome.tsx:228` (a _bordered_ button left transparent) — it lives inside a `<div className="window">` (`:204`), so it inherits the window fill and is a valid exception, not a miss. → _Post-merge: visual confirm the bordered button on `--window` over the grid reads correctly in dark theme._

### Findings (severity-tagged)

- **[INFO]** `tokens.css:1769-1770` `:last-child { border-bottom: none }` no longer needs `!important` (correctly dropped) and is now purely defensive — no rule sets a bottom border on those children. Harmless.
- **[INFO / pre-existing, out of scope]** `FilingNew.tsx` resume effect vs persona-reset effect ordering (detail in Fix 2 above) — switching persona while `?resume=` is in the URL would wipe the resumed draft. Predates PR-E; flagging only for awareness, not a blocker for this PR.
- **[NEEDS-CONFIRM — visual]** Fixes 1 and 5 are CSS/visual; the code-level reasoning is sound but the divider edge-to-edge bleed and the dark-theme solid-button contrast should be eyeballed in a browser post-merge.

### Verified clean (no action)

- **No dead code introduced** — every removed import/markup either still has a live consumer (`Link` in Analytics, `InfoTip` in FilingRecord) or was fully inline-styled (no orphan CSS).
- **Types sound** — synthetic `ClassifyResponse` and `headingTip?: ReactNode` both match their declared types; tsc clean.
- **Surgical** — every changed line traces to one of the five fixes; no refactors, no backend/core/citation/tax-figure changes, no AI-attribution markers.
...
# QA Review — PR-G1: Loading skeletons + tooltip overflow fix + full-bleed dividers `[FE]`

**Reviewed:** 27/06/26 · **Branch:** `feat/loading-skeletons` (uncommitted vs `main`) · **Reviewer:** QA (Opus/high)
**Scope:** `git diff main -- frontend/ docs/` + new `frontend/src/components/Skeleton.tsx`. 11 changed files, 1 new.

## Verdict: PASS (Approve with comments)

All three PR items are correct, in scope, and surgical. All three gates green. One LOW (pre-existing, not introduced here) divider-doubling observation on the two `.requirement-row` consumers that also carry an inline `border-bottom`, plus two NITs. Nothing blocking.

## Gate results

| Gate      | Command                            | Result                                                              |
| --------- | ---------------------------------- | ------------------------------------------------------------------- |
| Typecheck | `cd frontend && bunx tsc --noEmit` | PASS — exit 0, no errors                                            |
| Build     | `cd frontend && bun run build`     | PASS — `tsc -b && vite build`, 85 modules, 0 errors, built in 2.30s |
| Lint      | `bunx biome check frontend/src`    | PASS — 48 files checked, 0 errors, no fixes applied                 |

## Item 1 — Loading skeletons: PASS

- `Skeleton.tsx` exports `Skeleton` (rect), `SkeletonText` (n-line stack, last line 65% width), `SkeletonCard` (window + titlebar + body). All three are `aria-hidden="true"`. SkeletonCard wraps `SkeletonText` when `bodyHeight` is omitted; renders a single block when `bodyHeight` is set. Clean, minimal, no speculative API.
- `.skeleton` rule (`tokens.css` 1880-1886): `background: linear-gradient(90deg, var(--screen) 25%, var(--grid) 50%, var(--screen) 75%)`, `background-size: 200% 100%`, animated via `@keyframes skeleton-shimmer` on `background-position`. Uses design tokens `--screen` / `--grid` / `--radius` (all defined: tokens.css 16/15/30 light, 50/49 dark). No hard-coded colors.
- **Reduced-motion is FROZEN under BOTH paths**, mirroring the `.barber` exception pattern: `.reduce-motion .skeleton` (1889-1894, `animation:none !important` + static `--screen`) AND `@media (prefers-reduced-motion: reduce) .skeleton` (1896-1902). Correct — unlike `.barber`, the shimmer is decorative not functional, so freezing (not keeping it moving) is the right call.
- **All six pages + FilingNew converted, each gated on a loading flag (not permanent):** Dashboard (`loading &&` 213, content `!loading &&` 240), Analytics (`loading &&` 437, content `!loading && !error && totalCount>0` 490), Entity (early-return on `!entityReady || loading` 161, real content past 270), ObligationRadar (`(entityLoading || (!data && !displayError)) && !displayError` 373; inner `!data && !displayError` 414), FilingStudio (`loading &&` 279, content `!loading && records.length>0` 353), AuditAssistant (`filingsLoading &&` 600, content `!filingsLoading && filings.length>0` 656), FilingNew (`entityLoading &&` 486, content `!entityLoading && entity` 494).
- **Barber strips for in-progress ACTIONS preserved** — `.barber` JSX usages now exist only for classify/compute/pipeline progress; the diff removed `.barber` only from the six INITIAL-load placeholders. `.barber` CSS (incl. its reduced-motion keep-moving exception 1859-1861) untouched.
- Instant-resolve case: skeleton only mounts while the loading flag is true; a sync/cached resolve flips the flag before paint, so no skeleton flash — same gating as the prior barber placeholders, no regression.

## Item 2 — Tooltip overflow fix: PASS

- Bubble inline style gains `whiteSpace: 'normal'` + `overflowWrap: 'anywhere'` (Tooltip.tsx 119-120). `maxWidth` is still applied in `reposition` BEFORE measuring (`effectiveMaxWidth = Math.min(280, vw - 16)` set on `bubbleRef.style.maxWidth` at line 37), so `bubbleRect.width` reflects the wrapped box and the both-axis viewport clamp (top flip + bottom clamp 50-52, horizontal clamp 54-55) operates on the constrained width.
- Bubble is `position: fixed` (105) — renders against the viewport, so it is NOT clipped by the new `.req-list { overflow: hidden }` even when an InfoTip lives inside a row (relevant: ObligationRadar 409 InfoTip sits in a titlebar, not a clipped row, but the point holds generally).
- InfoTip API unchanged (`content`, `label`); `Tooltip` props unchanged. The three Analytics card-heading tooltips will now wrap at ≤280px on wide screens and at `vw-16` near 768px. Exact pixel rendering is browser-verifiable (see post-merge notes).

## Items 3 & 5 — Full-bleed dividers: PASS (with one LOW, pre-existing)

- `.requirement-row + .requirement-row` changed from `box-shadow: inset 0 1px 0 var(--ink)` to `border-top: 1px solid var(--ink)` (tokens.css 871-873); `.req-list` gains `overflow: hidden` (855-860). For the clean consumers that rely solely on the CSS rule (**FilingPipeline** rows 175/302-389, **CitationPanel** 48), this yields true edge-to-edge dividers with no last-row collision — the intended fix.
- **`overflow: hidden` clipping probe — no real regression:**
  - `kind-tag` badges self-clip (`.kind-tag { overflow:hidden; max-width:140px }` 915-926) and sit in normal flow inside the row's padding box — not clipped further by `.req-list`.
  - Tooltips/InfoTip anchored in a row are `position: fixed` — immune to ancestor `overflow:hidden`.
  - Wrapping text increases row height in normal flow (`overflow-wrap:anywhere` on toplines 890/898) — not an overflow, unaffected.
  - Only theoretical residual: a focus ring on an interactive element flush against the first/last row's clip edge. Rows have ≥12-15px padding so outlines stay inset; LOW risk, browser-verifiable.

## Findings

| #   | Severity           | Location                                       | Finding                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| --- | ------------------ | ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | Low (pre-existing) | `Dashboard.tsx:250`, `ObligationRadar.tsx:438` | These `.requirement-row` consumers set an inline `borderBottom: 'var(--border)'` on EVERY row (incl. the last). With the new `border-top` adjacent-sibling rule, adjacent rows now show inline `border-bottom` (row N) immediately above CSS `border-top` (row N+1) → a potential 2px double divider, and the last row's inline `border-bottom` can collide with the enclosing `.window` outline. NOT introduced by this PR (the inline borders predate it; the old inset-shadow already coexisted with them), but the swap to a solid `border-top: 1px solid var(--ink)` may make any doubling more visually prominent than the old subtle inset shadow. Note also progress.md's claim "last row has no border-bottom" is inaccurate for these two pages. Suggested fix (separate, optional): drop the inline `borderBottom` on these rows and let the CSS rule own the divider, OR scope it via `:not(:last-child)`. Non-blocking. |
| 2   | Low (nit)          | `ObligationRadar.tsx:373` vs `:401`/`:413`     | During the window where `entity` is loaded but `data` is still fetching, the outer skeleton block (373, fires on `!data`) AND the real "Filing Obligations" card with its inner skeleton (401 `entity &&` + 413 `!data &&`) can render simultaneously → two stacked skeleton sets. Pre-existing structure (the PR only swapped barber→Skeleton in both spots), self-heals when `data` arrives. Timing-dependent, browser-verifiable. Non-blocking.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 3   | Low (nit)          | `Auth.css:67-70, 152`                          | Quote-style reformat (`'` → `"` on `[data-theme="dark"]` and `content:""`) is biome auto-formatting, unrelated to the three PR items. Harmless and lint-driven; flagged only for surgical-change tidiness. Non-blocking.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |

No Medium/High findings. No type, boundary, or contract issues. No backend/core/citation/tax-figure changes (frontend-only). TRD untouched. No AI-attribution markers introduced.

## Partially code-verifiable (defer to post-merge browser smoke test)

- The three Analytics card-heading tooltips actually wrapping within `maxWidth` at a far-right card on BOTH wide and ~768px widths (code path correct; render pixel-verifiable).
- Skeleton shimmer freezing to a static muted block under reduced motion (both `.reduce-motion` toggle and OS-level `prefers-reduced-motion`).
- Full-bleed dividers being truly edge-to-edge with no left-edge gap on the clean consumers (FilingPipeline / CitationPanel), AND whether Finding #1's inline-border doubling is visible on Dashboard / ObligationRadar.
- The `overflow: hidden` focus-ring-at-row-edge edge case (Finding's only residual clip risk).

**Return to PM:** **APPROVE (with comments).** All three PR-G1 items are correct, surgical, in scope; all three gates clean (tsc 0, build 85 modules, biome 0 across 48 files). Reusable `Skeleton`/`SkeletonText`/`SkeletonCard` primitive is `aria-hidden`, token-driven, and frozen under BOTH `.reduce-motion` and `prefers-reduced-motion`; skeletons replaced the INITIAL-load placeholders on all six pages + FilingNew, each gated on a loading flag with real content after, and the in-progress-ACTION barber strips were left intact. Tooltip fix (`whiteSpace:normal` + `overflowWrap:anywhere`, `maxWidth` applied pre-measure, fixed-position bubble, both-axis clamp, unchanged InfoTip API) resolves the Analytics overflow. Dividers moved to `border-top` + `.req-list overflow:hidden`; no real clipping of kind-tag / tooltips / wrapping text. Only LOW/NIT findings, none blocking: (1) pre-existing inline `border-bottom` on Dashboard/ObligationRadar rows can double the new `border-top` divider — and progress.md's "last row has no border-bottom" is inaccurate there; (2) ObligationRadar can briefly stack two skeleton sets during the entity-loaded/data-pending window; (3) unrelated Auth.css quote reformat. Recommend the planned browser smoke test to confirm tooltip wrapping at far-right/768px, divider edge-to-edge + Finding #1 doubling, and skeleton freeze under reduced motion.

---

# QA Verdict — PR-G2: Filing-new upload doc-type picker + preview modal + back link + remove "entered directly · no AI"

**Date:** 28/06/26 · **Branch:** `feat/filing-upload-redesign` (uncommitted vs `main`) · **Reviewer:** QA
**Scope (frontend-only):** `frontend/src/pages/FilingNew.tsx`, `frontend/src/components/FilingPipeline.tsx`, `docs/progress.md`
**Verdict:** **APPROVE WITH COMMENTS** — all four items implemented, all gates pass; one Medium defense-in-depth item (XSS suppression justification is inaccurate; practical risk is low for the self-upload demo threat model) + two Low items, none blocking for the demo.

## Gates

| Gate                      | Command                                              | Result                                                         |
| ------------------------- | ---------------------------------------------------- | -------------------------------------------------------------- |
| Typecheck                 | `cd frontend && bunx tsc --noEmit`                   | **PASS** (exit 0, clean)                                       |
| Build                     | `cd frontend && bun run build`                       | **PASS** (85 modules, 0 errors, JS 400.63 kB / gzip 113.15 kB) |
| Lint/format               | `bunx biome check frontend/src` (full src, 48 files) | **1 error — pre-existing, NOT this PR**                        |
| Lint (changed files only) | `bunx biome check FilingNew.tsx FilingPipeline.tsx`  | **PASS** (0 errors)                                            |

`client.ts` biome nit is **pre-existing**: the file is not in `git diff main --name-only` (0 hits), and the error is a formatter object-wrap on `business_income` at `client.ts:351` — unrelated to this PR. Not a blocker.

## Item-by-item

1. **Upload doc-type picker (item 4, Option 1) — PASS.** Step A: two `aria-pressed` single-select option cards driven by `DOC_TYPE_META` (label + "what we extract" caption: "revenue & expense lines" / "all ledger account balances"). Step B revealed only once `docType` is set; dropzone copy + formats tailored per type; "Use sample document" wired to the per-type fixture. `SAMPLE_DOCS` now `Partial<Record<DocType,…>>` — all 3 personas have BOTH types, and all 6 fixtures exist on disk (`public/fixtures/`: 3 csv + 3 pdf). Graceful degradation present: a dedicated fallback "Use sample document" button (lines 1095-1117) renders when the picked type lacks a sample, and `handleUseSample` falls back to `Object.values(personaSamples)[0]`. Existing upload→extract→prefill-rows→draft-create flow preserved (`handleUpload`/`handleDrop`/`createDraftFiling` untouched in substance).

2. **Loaded-doc row + preview modal (item 4) — PASS with comments.** Doc-row (icon + filename + type/sample label + eye button) present; `DocPreviewModal` is a floating `<dialog open>` with blurred dim scrim (`backdropFilter: blur(6px)` + `WebkitBackdropFilter`). PDF→`<iframe>` (object URL for uploads, `/fixtures/…` for samples); CSV→inline table; XLSX→graceful "preview unavailable" note. A11y verified in code: Escape close (window keydown), backdrop-click close (full-inset close `<button>`), visible `×` close control, `modalRef.focus()` on open + prior-focus restore on close via `triggerRef`. Object-URL hygiene correct: previous URL revoked before creating a new one (629-632) AND on unmount (499-506) — no leak. See findings #1, #2.

3. **Back link (item 4) — PASS.** `&larr; Back to Filing Records` above the `New Filing` h1, `font-mono` / `--ink-soft` / `fontSize: 11`, routes to `/filing`. Uses the `&larr;` entity (← not em dash). Matches the `/filing/[id]` breadcrumb style in `FilingRecord.tsx:250-256`.

4. **Text removal (item 3) — PASS.** `Stage1Detail` in `FilingPipeline.tsx` no longer renders `entered directly · no AI`; the conditional collapses to `{!manual && <SovereignBadge … />}`. SovereignBadge path for AI-classified items intact; manual titlebar still shows "Line Items" cleanly; no orphaned vars/imports (diff is +1/-5).

## Reasoning probes

- **Fixture path that 404s:** `handleUseSample` checks `res.ok` and surfaces `Could not load the sample document: …`; an actual upload-then-preview of a 404 fixture path in the `<iframe>` would render the browser's native error page inside the modal (cosmetic only — sample fixtures all exist, so not reachable in the demo).
- **CSV quoted-field row:** **Finding #2** — naive `.split(',')` mis-splits quoted fields containing commas. All three trial-balance fixtures contain such a cell (e.g. acme line 10: `"Property, plant & equipment b/f (net)"`), so the CSV preview table WILL show a misaligned/extra cell on that row. Preview-only cosmetic; the real backend extraction is unaffected.
- **Scroll-lock on modal open:** **NOT implemented** — the modal does not lock body scroll; the page behind the blurred scrim can still scroll. Minor UX; non-blocking. (Logged in comments.)
- **Draft-resume after restructure:** Resume effect (466-483) sets `aiClassified=false` + `phase:'classified'`; `Stage1Detail` receives `manual={!aiClassified}=true` → renders "Line Items" (no badge). Lands on the classified stage correctly. PASS.

## Findings

| #   | Severity                      | Location                        | Finding                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| --- | ----------------------------- | ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **Medium** (defense-in-depth) | `FilingNew.tsx:88-105, 319-320` | `csvToTableHtml` interpolates each cell directly into an HTML string with no escaping, then renders via `dangerouslySetInnerHTML`. A CSV cell containing markup (e.g. `<img src=x onerror=…>`) executes in the preview. The `biome-ignore` justification "user-loaded CSV only, no XSS risk" is **inaccurate** — it IS injectable. Practical risk is LOW for this demo (user previews their OWN uploaded file in their OWN session → self-XSS only; no persistence, no other viewer). Suggested fix: HTML-escape each cell (`&`,`<`,`>`,`"`) before interpolation, and correct the ignore comment. Not blocking for the hackathon demo. |
| 2   | **Low** (cosmetic)            | `FilingNew.tsx:92-94`           | Naive CSV split breaks quoted fields with embedded commas; all 3 trial-balance fixtures trip this on the PPE row. Preview table only; extraction pipeline unaffected. Fix: minimal quoted-field-aware parse, or note the preview is best-effort.                                                                                                                                                                                                                                                                                                                                                                                        |
| 3   | **Low** (UX/nit)              | `FilingNew.tsx:614-654 vs 794`  | After a successful upload, `phase` flips to `'classified'` on the same tick `loadedDoc` is set; the Stage 01 entry card (gated on `phase==='idle'                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |     | 'error'`) unmounts, so the loaded-doc row + eye button are **not visible immediately after upload** — only after "Edit Line Items" returns to `idle`(which preserves`loadedDoc`). The preview feature IS reachable, but the "row appears after upload" intent only holds on the edit-return path. Browser-verifiable; consider surfacing the doc-row in the classified view too. Non-blocking. |

No type, boundary, or contract issues. Frontend-only — no `core/`/`api/`, no tax-figure or citation changes; TRD untouched. No fabricated-citation surface.

## Confirm in post-merge browser smoke test

- Modal a11y end-to-end: focus moves to the dialog on open and restores to the eye button on close; Escape + backdrop-click + `×` all dismiss.
- Blurred backdrop renders (`backdrop-filter` support) across target browsers.
- PDF `<iframe>` actually renders the sample/uploaded PDF (browser PDF viewer dependent).
- CSV preview misalignment on the PPE row (Finding #2) and any markup-cell behaviour (Finding #1).
