...
pytest -q                                         # expect: 40 passed (offline — uses FakeLLMClient, no key)
...
> Lives in `frontend/` (Vite 5 + React 18 + TypeScript + React Router 7, Bun). **In progress** — reuses the team's ProofRank devkit design system (token-CSS).
...
## 3. Environment (`.env` — copy from `.env.example`; never commit real secrets)
...
- **Frontend → Vercel:** root directory `frontend/`; set `VITE_API_URL` to the Render backend URL; Vite build → static SPA.
- **Backend → Render:** deploy the existing `Dockerfile`; set `LLM_*` (and optional `MYINVOIS_*`) as Render env vars; health check `/health`.
- **CI:** GitHub Actions runs `pytest` + builds the Docker image on push.
